...
func get_dialogue_response_async(callback: Callable) -> void:
	## Async dialogue: tries backend API, falls back to template.
...
func on_player_conversation_ended() -> void:
	...
	# No backend — store a simple summary locally
	var summary_parts: Array[String] = []
	for msg: Dictionary in history_copy.slice(0, mini(6, history_copy.size())):
	...
	if OS.is_debug_build():
		print("[ConvSummary] %s: Local summary stored (no backend)" % npc.npc_name)
	npc.unlock_conversation()
...
func _get_template_response() -> String:
	## Fallback template responses when backend is unavailable.
	if npc.energy < 20.0:
