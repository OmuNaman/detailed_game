...
func pick_gossip_for(other_npc: CharacterBody2D) -> Dictionary:
	...
	## NOTE: API version is async and uses pick_gossip_for_async() instead.
	## This synchronous version is the local fallback.
...
func share_gossip_with(receiver_npc: CharacterBody2D, original_memory: Dictionary) -> void:
	...
	if ApiClient.is_available():
		var body: Dictionary = {
			"sharer_name": npc.npc_name,
			"receiver_name": receiver_npc.npc_name,
			"memory_text": original_desc,
			"memory_importance": original_memory.get("importance", 3.0),
			"memory_valence": original_memory.get("emotional_valence", 0.0),
			"memory_actor": about,
			"gossip_hops": original_memory.get("gossip_hops", 0),
			"game_time": GameClock.total_minutes,
		}
		ApiClient.post("/gossip/share", body, func(response: Dictionary, success: bool) -> void:
			if success:
				# Apply local relationship effects (server doesn't manage relationships)
				...
				if OS.is_debug_build():
					print("[Gossip API] %s told %s about %s (hop %d)" % [npc.npc_name, receiver_npc.npc_name, about, hop_count])
		)
		return
	...
	receiver_npc._add_memory_with_embedding(
		gossip_desc, "gossip", about,
		[npc.npc_name, receiver_npc.npc_name, about] as Array[String],
		receiver_npc._current_destination, npc._current_destination,
		gossip_importance, original_memory.get("emotional_valence", 0.0)
	)
	...
	npc._add_memory_with_embedding(
		...
		"gossip_shared", receiver_npc.npc_name,
		[npc.npc_name, receiver_npc.npc_name] as Array[String],
		npc._current_destination, npc._current_destination, 2.0, 0.0
	)
...
func detect_third_party_mentions(speaker_name: String, line_text: String, listener: CharacterBody2D) -> void:
	...
	if ApiClient.is_available():
		var body: Dictionary = {
			...
			"all_npc_names": Array(all_names),
			"player_name": player_name,
			"game_time": GameClock.total_minutes,
		}
		ApiClient.post("/gossip/detect-mentions", body, func(response: Dictionary, success: bool) -> void:
			if success and OS.is_debug_build():
				var mentions: Array = response.get("mentions", [])
				for m: Variant in mentions:
					if m is Dictionary:
						print("[Diffusion API] %s heard %s mention %s" % [listener.npc_name, speaker_name, m.get("mentioned_name", "")])
		)
		return
