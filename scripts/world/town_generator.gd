...
func _create_navigation_region() -> void:
	...
	# Let the engine auto-detect walkable areas from static colliders.
	# Walls, roofs, and water already have StaticBody2D nodes → obstructions.
	# Floor and door tiles have NO colliders → walkable. No manual outlines needed.
	...
	nav_poly.baking_rect = Rect2(0, 0, MAP_WIDTH * TILE_SIZE, MAP_HEIGHT * TILE_SIZE)

	nav_region.navigation_polygon = nav_poly
	add_child(nav_region)

	# Bake synchronously so the navmesh is ready before NPCs start pathfinding
